...
bool s21_parser(const char *filename, Model *model) {
  ...
  rewind(file);
  model->vertices =
      (double *)calloc((model->total_vertices + 1) * 3, sizeof(double));
  ...
  if (suc) {
    ...
    while (fgets(line, sizeof(line), file)) {
      if (line[0] == 'v' && line[1] == ' ') {
        ...
        model->polygons[id_facet].numbers_of_vertices =
            (int *)malloc(count_of_vertices * sizeof(int));
...
void check_extremum(Model *model, int id_vertex) {
  for (int i = 0; i < 3; i++) {
    if (model->vertices[id_vertex * 3 + i] > model->max[i])
      model->max[i] = model->vertices[id_vertex * 3 + i];
    if (model->vertices[id_vertex * 3 + i] < model->min[i])
      model->min[i] = model->vertices[id_vertex * 3 + i];
  }
}
